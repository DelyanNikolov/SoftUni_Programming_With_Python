...
class SummitQuestManagerApp:
    ...
    def get_statistics(self):
        qualified_climbers = filter(lambda c: len(c.conquered_peaks) > 0, self.climbers)
        result_climbers = sorted(qualified_climbers, key=lambda x: (-len(x.conquered_peaks), x.name))

        total_peaks_climbed = len({p for c in result_climbers for p in c.conquered_peaks})

        result = f"Total climbed peaks: {total_peaks_climbed}\n"
        result += "**Climber's statistics:**\n"
        result += '\n'.join(str(cl) for cl in result_climbers)
